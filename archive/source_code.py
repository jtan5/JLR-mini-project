...
# food_dict autogenerated
food_dict = dict(zip(choice_list, [drinks, cold_food, hot_food]))
...
def menu3():
    ...
    prod_list = [key for key, val in food_dict.items()]
    ...
    for index, val in enumerate(choice_list):
        ...
        if choice_tuple[0] == index:
            ...
            for index, val in enumerate(food_dict[choice_tuple[2]]):
                print(f"{index} - {val}")
...
def menu4():
    ...
    # This coding below is to work out which category of item to add
    for index, val in enumerate(choice_list):  # drinks, hot_food, cold food
        #print(f"Assessing if choice_tuple is equal to {index} - {val}")
        if choice_tuple[0] == index:
            # debug line
            # print(
            ...
            for index, val in enumerate(food_dict[choice_tuple[2]]):
                print(f"{index} - {val}")
            print(f"The list of products {food_dict[choice_tuple[2]]}")
            edit_list = food_dict[choice_tuple[2]]
            ...
            new_prod_name = input(
                f"Instead of {food_dict[desired_cat[2]][choice_tuple[0]]} \nWhat would you like this product to be called instead?\n")
            food_dict[desired_cat[2]][choice_tuple[0]] = new_prod_name
            #edit_list[index] = new_prod_name
            # food_dict[choice_tuple[2]]
            # food_dict[choice_tuple[2]].append(new_prod)
            os.system('cls')
            print("The new product list is as below")
            for index, val in enumerate(food_dict[desired_cat[2]]):
                print(f"{index} - {val}")
            input("Press any key to return to main menu")
            menu2()
        else:
...
def menu5():
    ...
    # prod_list is the keys of food_dict
    prod_list = [key for key, val in food_dict.items()]
    question = "What category would you like to create?"
    # initialising choice_tuple to run the while loop on first call;
    choice_tuple = (99, 0, 0)
    # while loop stops any invalid entries passing through further down
    while choice_tuple[0] >= choice_tuple[1]:
    ...
    for index, val in enumerate(choice_list):  # drinks, hot_food, cold food
        ...
        if choice_tuple[0] == index:
            ...
            food_dict[desired_cat[2]] = edit_list_new
