...
import time
import os
#from source.source_code_wk2 import menu1
import source.access_func as af
#from replit import clear
#import access_func as af
sys.path.append()
#sys.path.insert(0, '/source')
# from source_code_wk2 import menu1
...
def printdynamic(selection, question, art, add_option=0):
    os.system('clear')
    print(art)
    print(question)
    for index, val in enumerate(selection):
        print(f"{index} - {val}")
    if add_option == 1:
        print("99 - Return to main menu")

    choice2 = int(input("Please enter a valid option as listed above\n"))
    if (choice2) < len(selection):
        selecteditem = selection[choice2]
        return choice2, len(selection), selecteditem
    else:
        return choice2, len(selection), "Invalid selection"

#***FIRST MENU TO CHOOSE THEM ALL! ***
option1 = ['Quit', 'View all products on menu', 'View all couriers', 'v']
...
def menu1():
    ...
    while choice_tuple[0] >= choice_tuple[1] or choice_tuple[0] == None or choice_tuple[0] == "":
...
def menu5(): #MENU TO DELETE EXISTING PRODUCTS
